  ...
  methods: {
    ...
    isMonth(dateFrom, dateTo) {
      const startOfPeriod = this.$moment(dateFrom)
        .startOf("month")
        .format("YYYY-MM-DD");
      const endtOfPeriod = this.$moment(dateTo)
        .endOf("month")
        .format("YYYY-MM-DD");
      const diference = this.$moment
        .duration(this.$moment(endtOfPeriod).diff(this.$moment(startOfPeriod)))
        .asDays();

      return (
        this.$moment(startOfPeriod).isSame(this.$moment(dateFrom)) &&
        this.$moment(endtOfPeriod).isSame(this.$moment(dateTo)) &&
        diference > 26 &&
        diference < 32
      );
    },
