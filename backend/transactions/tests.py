...
class BudgetTests(test.APITestCase):
    ...
    def test_budget_creating_and_deleting_by_admin_user(self):
        """
        Admin user must be able to create budgets in all wallets
        """
        for category_id in range(1, 10):
            data = {
                "name": "test",
            ...
            self.client.credentials(HTTP_AUTHORIZATION='JWT ' + self.token_for_admin)
            response = self.client.post(self.budgets_url, data=data, format="json")
            self.assertEqual(response.status_code, status.HTTP_201_CREATED)
            
            response = self.client.delete(self.budgets_url + str(response.data["id"]) + "/", format="json")
            self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)

    def test_budget_creating_and_deleting_by_basic_user(self):
        """
        Basic user must be able to create budgets only in his wallets
        """
        for category_id in range(1, 10):
            data = {
                "name": "test",
        ...
        for budget_id in range(2, 10):
            self.client.credentials(HTTP_AUTHORIZATION='JWT ' + self.token_for_user_panzelva)
            response = self.client.delete(self.budgets_url + str(budget_id) + "/", format="json")
            if budget_id in range(2, 5):
                self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
            else:
                self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)

    def test_budget_creating_and_deleting_by_unauth_user(self):
        """
        Unauth user must NOT be able to create budgets
        """
        for category_id in range(1, 10):
            data = {
                "name": "test",
            ...
            response = self.client.delete(self.budgets_url, format="json")
            self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
